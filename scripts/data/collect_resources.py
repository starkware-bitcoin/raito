...
BUILTIN_TYPES = [
    "range_check_builtin", "bitwise_builtin", "poseidon_builtin", 
    "pedersen_builtin", "range_check96_builtin", "ecdsa_builtin", 
    "ec_op_builtin", "keccak_builtin", "mul_mod_builtin", 
    "add_mod_builtin", "output_builtin"
]
...
class ResourceCollector:
    ...
    def run_test(self, test_file: str, nocapture: bool, forceall: bool, 
        ...
        arguments_file = f"{os.path.dirname(test_file)}/.arguments-{os.path.basename(test_file)}"
        ...
        try:
            ...
            result = subprocess.run([
                ...
                "--print-resource-usage", "--arguments-file", arguments_file
    ...
    def run(self, test_files: List[str], nocapture: bool, forceall: bool, 
            lightonly: bool, collect_only: bool) -> None:
...
def main():
    parser = argparse.ArgumentParser(description="Collect resource usage data from Scarb tests")
    ...
    parser.add_argument("--forceall", action="store_true", help="Force run all tests including ignored ones")
    parser.add_argument("--collect-only", action="store_true", help="Only collect data, don't run tests")
    parser.add_argument("test_files", nargs="*", help="Specific test files to run")
    
    args = parser.parse_args()
    
    collector = ResourceCollector()
    collector.run(args.test_files, args.nocapture, args.forceall, 
                  args.lightonly, args.collect_only)
