...
def get_latest_block_height() -> int:
    ...
    try:
        ...
        for item in latest_shard_dir.iterdir():
            if item.is_file() and item.name.startswith("block_") and item.name.endswith(".json"):
                try:
                    # Extract block height from filename (block_XXXXX.json)
                    block_height = int(item.name[6:-5])  # Remove "block_" prefix and ".json" suffix
                    block_files.append(block_height)
                except ValueError:
                    continue
        
        if not block_files:
            logger.error(f"No block files found in latest shard directory: {latest_shard_dir}")
            raise FileNotFoundError(f"No block files found in latest shard directory: {latest_shard_dir}")
